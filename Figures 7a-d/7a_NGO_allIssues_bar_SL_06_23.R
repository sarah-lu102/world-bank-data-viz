...
for(i in 1:9){
  ...
  for(j in 1:130){
    ...
    if(j!= 34){
      ...
      if(dat[j, "issue_secondary"] == i && dat[j, "foreign_ngo"] == 0 && dat[j, "domestic_ngo"] == 0){
        ncount = ncount + 1
      }
      if(dat[j, "issue_tertiary"] == i && dat[j, "foreign_ngo"] == 0 && dat[j, "domestic_ngo"] == 0){
        ncount = ncount + 1
      }
...
p= ggplot(data = IP_df, aes(x = Issue, y = Num_cases, fill = NGO_type))+
  ...
  #plot title
  labs(title = "Inspection Panel- All Issues")
...
q= ggplot(data = CAO_df, aes(x = Issue, y = Num_cases, fill = NGO_type))+
  ...
  #plot title
  labs(title = "Compliance Advisor/Ombudsman- All Issues")
